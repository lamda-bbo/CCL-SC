...
model_names = ("vgg16","vgg16_bn","resnet34","resnet18", "EfficientNet", "resnext50_32x4d", "regnet_x_400mf", "regnet_x_800mf", "regnet_x_1_6gf", "shufflenet_v2_x1_0", "shufflenet_v2_x1_5")

parser = argparse.ArgumentParser(description='Selective Classification for Self-Adaptive Training')
parser.add_argument('-d', '--dataset', default='cifar10', type=str, choices=['cifar10', 'celeba', 'svhn', 'imagenet100', 'imagenet_subset', 'imagenet', 'cars', 'food', 'cifar100'])
...
def main():
    ...
    if args.dataset == 'cifar10':
        ...
        trainset_dev = dataset(root='~/datasets/CIFAR10', train=True, download=True, transform=transform_test)
        ...
        trainset_dev = dataset(root='~/datasets/CIFAR100', train=True, download=True, transform=transform_test)
        ...
        trainset_dev = dataset(root='~/datasets/SVHN', split='train', download=True, transform=transform_test)
        ...
        if args.mode == 'tuning':
            ...
            trainset_dev = large_dataset_utils.ImageNet_Dataset(train_data_dir, transform=test_trsfm)
        ...
        trainset_dev = large_dataset_utils.Celeba(root='~/datasets/', split='train', target_type='attr', transform=val_transform, download=True)
    # DataLoaders
    ...
    devloader = torch.utils.data.DataLoader(trainset_dev, batch_size=args.test_batch, shuffle=True, num_workers=args.workers)
    
    
    # Model
    print("==> creating model '{}'".format(args.arch))
    
    if args.dataset != "svhn" and args.arch != 'vgg16_bn':
        model = non_cifar_models.__dict__[args.arch](num_classes=num_classes if args.loss == 'ce' or args.loss == 'log_ml' or args.loss == 'csc' else num_classes+1)
    else:
        model = models.__dict__[args.arch](num_classes=num_classes if args.loss == 'ce' or args.loss == 'log_ml' or args.loss == 'csc'else num_classes+1, input_size=input_size)
    ...
    # if args.arch == 'vgg16_bn':
    #     print(list(model.module.classifier._modules.items())[0][1].register_forward_hook(hook_fn))
    #     print("vgg16_bn_hook")
    # else:
    #     print(model.module.fc._modules.items())
    ...
    if args.evaluate:
        ...
        elif args.arch == 'resnet34' or'resnet18' or 'resnet50':
            print(model.avgpool)
            print(model.avgpool.register_forward_hook(hook_fn))
        if use_cuda: model = model.cuda()
        test(devloader, testloader, model, criterion, args.epochs, use_cuda, evaluation=True)
        return
    ...
    # train
    archive = moco.builder_sel_out.MoCo(
        args.moco_dim,
        args.moco_k,
        args.moco_m,
        args.moco_t,
...
def train(trainloader, archive, model, model_k, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx,  batch_data in tqdm(enumerate(trainloader)):
        ...
        # print('outputs:', outputs,outputs.size(), accuracy(outputs.data, targets.data, topk=(1,))[0])
        ...
        # print('hook: ', hidden_features, hidden_features.shape)
        # print(outputs_feature.shape)
        # print(nn.Sequential(*list(model.module.classifier.children())[:-1]))
        if args.arch == 'vgg16_bn':
        ...
        if epoch >= args.pretrain:
            if args.arch != 'vgg16_bn':
                ...
                moco_error, loss2 = archive(torch.flatten(hidden_features, 1), torch.flatten(hidden_features_k, 1), targets, outputs, outputs_k, epoch + 1, args.pretrain)
                ...
                moco_error, loss2 = archive(outputs_projection, hidden_features_k, targets, outputs, outputs_k, epoch + 1, args.pretrain)
            if args.loss == 'gambler':
                ...
                # print(loss2)
                loss = criterion(outputs, targets) + loss2 * reward
                
            else:
                loss = criterion(outputs, targets)
        else:
            moco_error = 1/ num_classes
            if args.arch != 'vgg16_bn' and epoch >= args.pretrain - 10:
                moco_error = archive(torch.flatten(hidden_features, 1), torch.flatten(hidden_features_k, 1), targets, outputs, outputs_k, epoch + 1, args.pretrain)
            else:
                if epoch >= args.pretrain - 10:
                    moco_error = archive(outputs_projection, hidden_features_k, targets, outputs, outputs_k, epoch + 1, args.pretrain)
...
def test(devloader, testloader, model, criterion, epoch, use_cuda, evaluation = False):
    global best_acc

    # whether to evaluate uncertainty, or confidence
    if evaluation:
        evaluate(devloader, testloader, model, use_cuda)
        return
...
def evaluate(devloader, testloader, model, use_cuda):
    model.eval()
    abortion_results = [[],[]]
    ...
    if True:
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(devloader):
                inputs, targets = batch_data[:2]
                if use_cuda:
                    inputs, targets = inputs.cuda(), targets
                output_logits = model(inputs)
                if args.arch != 'vgg16_bn':
                    feature_train.extend(list(torch.flatten(hidden_features, 1)))
                else:
                    feature_train.extend(list(hidden_features))
                ...
                prediction_list_train.extend(list(predictions))
                correct_list_train.extend(list( predictions.eq(targets.data) ))
                target_list.extend(list(targets.data))
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(testloader):
            ...
            elif args.loss == 'log_ml' or args.loss == 'csc' or args.loss == 'csc_entropy':
    ...
    average_features_train, class_counts_train, error_average_features_train = get_feature_avg(correct_list_train, prediction_list_train, feature_train)
    average_features, class_counts, _ = get_feature_avg(correct_list[:args.num_valid], prediction_list[:args.num_valid], feature[:args.num_valid])
    ...
    sim_abortion_by_train[0], sim_abortion_class_by_train, _, _ = cal_sim(average_features_train, feature[args.num_valid:], prediction_list[args.num_valid:], correct_list[args.num_valid:])
    sim_abortion_by_train[1] = sr_results[1][args.num_valid:]
    sim_abortion_by_train_error, _, _, _ = cal_sim(error_average_features_train, feature[args.num_valid:], prediction_list[args.num_valid:], correct_list[args.num_valid:])
    ...
    with open(os.path.join(save_path, end), 'w') as file:
        ...
        for idx, _ in enumerate(abortion_results):
            print('{:.3f}'.format((1 - abortion_results[idx][1])*100))
            file.write('\n{:.3f}'.format((1 - abortion_results[idx][1])*100))
        ...
        for idx, _ in enumerate(sr_results):
            ...
            print('{:.3f}'.format((1 - sr_results[idx][1])*100))
            file.write('\n{:.3f}'.format((1 - sr_results[idx][1])*100))
        if args.loss == "log_ml":
        ...
        if True:
            sim_scores_by_train, sim_correct_by_train = sim_abortion_by_train[0].cpu(), torch.stack(sim_abortion_by_train[1]).cpu()
            sim_results = []
            bisection_method(sim_scores_by_train, sim_correct_by_train, sim_results)
            file.write("\nsim_by_train\tLogit\tTest\tCoverage\tError")
            print("\nsim_by_train\tLogit\tTest\tCoverage\tError")
            for idx, _ in enumerate(sim_results):
                print('{:.2f}'.format((1 - sim_results[idx][1])*100))
                file.write('\n{:.2f}'.format((1 - sim_results[idx][1])*100))
        # sim - not sim
        if True:
            sim_scores_by_train, sim_correct_by_train = (sim_abortion_by_train[0] - 0.05*sim_abortion_by_train_error).cpu(), torch.stack(sim_abortion_by_train[1]).cpu()
            sim_results = []
            bisection_method(sim_scores_by_train, sim_correct_by_train, sim_results)
            file.write("\nsim-notsim\tLogit\tTest\tCoverage\tError")
            print("\nsim-notsim\tLogit\tTest\tCoverage\tError")
            for idx, _ in enumerate(sim_results):
        ...
        if True:
            sim_scores, sim_correct = sim_abortion_by_train[0].cpu(), torch.stack(sim_abortion_by_train[1]).cpu()
            sim_scores2 = (- sim_abortion_by_train_error)
...
def get_feature_avg(correct_list, prediction_list, features):
    ...
    error_average_features = [np.zeros_like(features[0].cpu().numpy()) for _ in range(num_classes)]
    ...
    for feature, prediction, correct in data:
        if correct:
            ...
            error_average_features[prediction.item()] += feature.cpu().numpy()
    ...
    print('correct:', class_counts)
    print('error:', error_class_counts)

    return average_features, class_counts, error_average_features
...
def cal_sim(average_features, features, prediction_list, correct_list):
    ...
    print(np.array(sim_class) / np.array(class_count))
...
if __name__ == '__main__':
    ...
    for i in range(len(reward_list)): 
        ...
        if args.loss == 'csc' or args.loss == 'csc_entropy'or args.loss == 'csc_sat_entropy':
            if args.warmup:
